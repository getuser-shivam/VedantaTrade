...
router.get('/inventory/:centerId', simpleAuth, async (req: Request, res: Response) => {
  try {
    ...
    const [allocations, total] = await Promise.all([
      prisma.$queryRaw`SELECT ia.*, p.item_name as product_name, p.mrp, p.stock_quantity
...
router.post('/inventory/allocate', simpleAuth, async (req: Request, res: Response) => {
  try {
    const { product_id, center_id, quantity } = req.body;

    // Check if allocation already exists
    const existingAllocation = await prisma.$queryRaw`SELECT id FROM inventory_allocations 
    ...
    const product = await prisma.$queryRaw`SELECT stock_quantity FROM inventory_items WHERE item_id = ${product_id}`;
    ...
    const allocation = await prisma.$queryRaw`INSERT INTO inventory_allocations 
    ...
    await prisma.$queryRaw`UPDATE inventory_items 
...
router.get('/routes', simpleAuth, async (req: Request, res: Response) => {
  try {
    ...
    const [routes, total] = await Promise.all([
      prisma.$queryRaw`SELECT dr.*, dc.name as center_name, u.name as driver_name, u.phone as driver_phone
