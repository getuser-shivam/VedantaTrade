...
class AuthenticationProvider extends ChangeNotifier {
  ...
  bool get needsPasswordChange => _currentUser?.passwordChangedAt != null && 
  ...
  Future<void> signUp(String email, String password, String firstName, String lastName, String phoneNumber) async {
    ...
    try {
      ...
      final user = await _repository.signUp(email, password, firstName, lastName, phoneNumber);
  ...
  Future<void> verifyMfaToken(String token) async {
    ...
    try {
      final isValid = await _repository.verifyMfaToken(_currentUser!.id, token);
  ...
  Future<void> resendMfaToken() async {
    ...
    try {
      final token = await _repository.generateMfaToken(_currentUser!.id);
      _setMfaToken(token);
    } catch (e) {
  ...
  Future<void> verifyEmail(String token) async {
    try {
      final isValid = await _repository.verifyEmail(token);
  ...
  Future<void> _generateMfaToken(String userId) async {
    try {
      final token = await _repository.generateMfaToken(userId);
      _setMfaToken(token);
    } catch (e) {
