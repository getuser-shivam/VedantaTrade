...
class EnhancedAuthProvider extends ChangeNotifier {
  ...
  Future<bool> setupTwoFactorAuthentication() async {
    try {
      final secret = await authService.generateTwoFactorSecret();
  ...
  Future<bool> verifyTwoFactorCode(String code) async {
    try {
      final secret = await _storage.read(key: 'two_factor_secret');
      if (secret == null) return false;

      final isValid = await authService.verifyTwoFactorCode(secret, code);
      if (isValid) {
